...
import { account, callAction, ID } from '@/lib/appwrite'
...
export default function RegisterScreen() {
  ...
  const handleRegister = async () => {
    ...
    setLoading(true)
    try {
      await account.create(ID.unique(), email.trim(), password, username)
      await account.createEmailPasswordSession(email.trim(), password)

      // Profile is created server-side so approved/isAdmin/xp can't be set by the client.
      await callAction('initProfile', { username, team })
